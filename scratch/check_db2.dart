...
void main() async {
  ...
  final res = await supabase.from('reservations').select().limit(5);
  print(res);

  print('Checking restaurants schema...');
  final rest = await supabase.from('restaurants').select().limit(1);
  if (rest.isNotEmpty) {
    print('Restaurant columns: ${rest[0].keys.join(", ")}');
  }
}
